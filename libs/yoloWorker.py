...
class YoloWorker(QObject):
    ...
    def _predict(self, img_path):
        ...
        results = self.model.predict(img, conf=self.conf, imgsz=self.imgsz,
                                     rect=True, verbose=False, workers=0)
        if not results:
            return []
        ...
        def get_class_name(idx):
            if isinstance(names, dict):
                if idx in names: return names[idx]
                elif str(idx) in names: return names[str(idx)]
                elif int(idx) in names: return names[int(idx)]
